...
/**
 * Translates exceptions raised by the vehicle-service into consistent JSON error
 * responses.
 *
 * <ul>
 *   <li>vehicle not found       -&gt; 404</li>
 *   <li>duplicate vehicle number -&gt; 409</li>
 *   <li>invalid state transition -&gt; 409</li>
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleNotFound(VehicleNotFoundException ex, HttpServletRequest request) {
        return build(HttpStatus.NOT_FOUND, ex.getMessage(), request, null);
    }

    @ExceptionHandler(DuplicateVehicleNumberException.class)
    public ResponseEntity<ApiError> handleDuplicate(DuplicateVehicleNumberException ex,
                                                    HttpServletRequest request) {
        return build(HttpStatus.CONFLICT, ex.getMessage(), request, null);
    }

    @ExceptionHandler(VehicleStateException.class)
    public ResponseEntity<ApiError> handleState(VehicleStateException ex, HttpServletRequest request) {
        return build(HttpStatus.CONFLICT, ex.getMessage(), request, null);
    }
