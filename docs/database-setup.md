...
> (`spring.jpa.hibernate.ddl-auto=update` for `user-service`). Production should
> replace this with explicit Flyway/Liquibase migrations. The sections below
> document what the service expects so the database can also be created by hand.
...
The Config Server assembles the JDBC URL as
...
Other services will own their own tables (vehicles, parking spaces, bookings,
payments) and will be documented as they are implemented.
